...
Kao što je već i pomenuto, aktivaciona funkcija je funkcija koje se primenjuje nad linearnom kombinacijom ulaza skaliranog težinama. Aktivaciona funkcija često uzima oblik sigmoidne funkcije:
$$\begin{aligned}
...
\hat{y} = \sigma(z_2),
\end{aligned}$$
gde $\hat{y}$ predstavlja vektor verovatnoća $\hat{Y_i}=P\{Y_i=1\}$.
...
Pozabavimo se sada odabirom koeficijenata matrica težina $W_{in}$ i $W_{out}$. Kod linearne regresije smo parametre ocenjivali metodom maksimalnih kvadrata. Slično, težine u matricama težina ocenjivaćemo (ČIME???).

Poznat nam je logaritamski oblik funkcije maksimalne verodostojnosti za Bernulijevu raspodelu (tj. klasifikator u 2 klase):(OVDE ISPRAVI ZA NASU FJUUUUU!!!!!!!)
$$\begin{aligned}
l = \sum_{i}\Big(y_i \log \hat{y_i}+(1-y_i)\log(1-\hat{y_i})\Big)
\end{aligned}$$

Da bismo maksimizirali funkciju $l=f(W)$ koristićemo algoritam opadajućeg gradijenta. Opadajući gradijent je optimizacioni algoritam koji pronalazi lokalni minimum funkcije tako što izvršava više koraka proporcionalnih negativnoj vrednosti gradijenta odgovarajuće funkcije.
...
Napomenimo da je odabir brzine učenja $\gamma$ jako bitan kako bi algoritam konvergirao ka stvarnom minimumu funkcije!
...
Primetimo da se vrednosti novog vektora sumiraju na 1, te ih stoga možemo koristiti kao raspodelu verovatnoće. Za vrednost aproksimacije se uzima kategorija koja odgovara izlazu sa najvišom vrednošću. 
